...
def compile_spec(
    ...
    """
    # Resolve version target: CLI flag overrides spec.yaml target:
    active_target = cli_target or spec.target
    version_overrides = {}
    if active_target:
    ...
    # Inject version-specific co-dependencies (e.g. cert-manager, jobset for RHOAI 3.x)
    for co_dep in version_overrides.get('co_dependencies', []):
        raw_ops.add(co_dep)
    ...
    # use_vault when any of: vault explicitly enabled, block-level secrets exist,
    # or top-level secrets exist (provider keys, SSH keys, etc.)
    use_vault = spec.vault_enabled or bool(all_secrets) or bool(spec.top_level_secrets)
    ...
    config = {
        ...
        'spec_dir': spec_dir,
        # Top-level secrets: → values-secret.yaml.template (VP v2 format)
        'top_level_secrets': spec.top_level_secrets,
        # Version overrides from --target or spec target: field
        'version_overrides': version_overrides,
        # Docs: → processed into vp-out/ and/or qs-out/
        'docs': spec.docs,
