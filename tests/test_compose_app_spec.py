...
class TestCustomComponentNamespace:
    ...
    def test_extra_value_files_emits_stub_when_source_missing(self, tmp_path):
        out = _compose(tmp_path, _CUSTOM_NS_SPEC)
        stub = out / 'overrides' / 'app-two-overrides.yaml'
        assert stub.exists()
        assert 'Stub generated by quickpat compose' in stub.read_text()
...
# ── Custom component override file copy / extraValues ─────────────────────────
...
custom:
  my-app:
    description: App with source-layer override
    source:
      chart: charts/my-app
    extraValueFiles:
      - /overrides/my-app.yaml
"""

_OVERRIDE_EXTRA_VALUES_SPEC = """\
apiVersion: supplychain/v1alpha1
kind: ApplicationSpec
metadata:
...
custom:
  my-app:
    description: App with inline extraValues
    source:
      chart: charts/my-app
    extraValues:
      inference:
...
class TestCustomComponentOverrides:
    def test_copies_source_override_into_vp_out(self, tmp_path):
        overrides = tmp_path / 'overrides'
        overrides.mkdir()
        source = overrides / 'my-app.yaml'
        source.write_text('sandboxName: from-source\ninference:\n  provider: custom\n')

        out = _compose(tmp_path, _OVERRIDE_COPY_SPEC)
        dest = out / 'overrides' / 'my-app.yaml'
        assert dest.exists()
        assert dest.read_text() == source.read_text()

    def test_recompose_preserves_source_override_contents(self, tmp_path):
        overrides = tmp_path / 'overrides'
        overrides.mkdir()
        source = overrides / 'my-app.yaml'
        contents = 'sandboxName: alice-saw\ninference:\n  model: keep-me\n'
        source.write_text(contents)

        out1 = _compose(tmp_path, _OVERRIDE_COPY_SPEC)
        assert (out1 / 'overrides' / 'my-app.yaml').read_text() == contents

        # Mutate vp-out (simulating a stale hand-edit) then re-compose from source
        (out1 / 'overrides' / 'my-app.yaml').write_text('stale: true\n')
        out2 = _compose(tmp_path, _OVERRIDE_COPY_SPEC)
        assert (out2 / 'overrides' / 'my-app.yaml').read_text() == contents

    def test_extra_values_writes_override_and_references_it(self, tmp_path):
        out = _compose(tmp_path, _OVERRIDE_EXTRA_VALUES_SPEC)
        apps = _read_yaml(out / 'values-prod.yaml')['clusterGroup']['applications']
        assert apps['my-app']['extraValueFiles'] == ['/overrides/my-app.yaml']

        data = _read_yaml(out / 'overrides' / 'my-app.yaml')
        assert data['inference']['provider'] == 'custom'
        assert data['inference']['model'] == 'test-model'
        assert data['accessControl']['owner'] == 'alice'

    def test_extra_values_takes_precedence_over_source_file(self, tmp_path):
        overrides = tmp_path / 'overrides'
        overrides.mkdir()
        (overrides / 'my-app.yaml').write_text('from: source-file\n')

        # Both extraValues and extraValueFiles pointing at the same path —
        # inline extraValues must win over the source-layer file.
        spec = """\
apiVersion: supplychain/v1alpha1
kind: ApplicationSpec
metadata:
...
custom:
  my-app:
    ...
    extraValues:
      inference:
        provider: from-extraValues
"""
        out = _compose(tmp_path, spec)
        data = _read_yaml(out / 'overrides' / 'my-app.yaml')
        assert data.get('from') is None
        assert data['inference']['provider'] == 'from-extraValues'

    def test_missing_source_warns_and_stubs(self, tmp_path):
        spec_file = tmp_path / 'spec.yaml'
        spec_file.write_text(_OVERRIDE_COPY_SPEC)
        result = compose_from_spec(str(spec_file), output_dir=str(tmp_path / 'out'))
        assert result.success
        stub = tmp_path / 'out' / 'overrides' / 'my-app.yaml'
        assert stub.exists()
        assert 'Stub generated' in stub.read_text()
        assert any('overrides/my-app.yaml' in w for w in result.warnings)
...
class TestVP2SecretTemplate:
    ...
    def test_values_global_has_ssh_public_key_placeholder(self, tmp_path):
        # Specs with an SSH keypair secret must emit global.sshPublicKey in
        # values-global.yaml so generate-keys.sh's sed patch has a target.
        # Without it the patch silently no-ops and sandbox VMs get no key.
        out = _compose(tmp_path, _SECRETS_SPEC)
        data = _read_yaml(out / 'values-global.yaml')
        assert 'sshPublicKey' in data['global']
        assert data['global']['sshPublicKey'] == ''

    def test_values_global_omits_ssh_public_key_without_ssh_secret(self, tmp_path):
        spec = """\
apiVersion: supplychain/v1alpha1
kind: ApplicationSpec
metadata:
...
secrets:
  - name: anthropic
    vault_path: no-ssh/anthropic
    fields:
      - name: api_key
"""
        out = _compose(tmp_path, spec)
        data = _read_yaml(out / 'values-global.yaml')
        assert 'sshPublicKey' not in data['global']
